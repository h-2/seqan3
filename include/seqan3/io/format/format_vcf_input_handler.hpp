...
enum class variant_file_special_value
{
    missing,    //!< "."
    unknown,    //!< "*"
};

using variant_file_allele = std::variant<variant_file_special_value,
                                         std::vector<dna5>,       // regular allele
                                         std::string>             // anything else, e.g. breakpoint-string


struct variant_file_header
{
    //TODO implement me
};
...
private:
    ...
    /* RAW RECORD HANDLING*/
    using format_fields = fields<field::chrom,
                                 field::pos,
                                 field::id,
                                 field::ref,
                                 field::alt,
    ...
    void read_raw_record()
    {
        ...
        char * end_qual   = (*file_it).fields[7].data() + (*file_it).fields[7].size();
    ...
    variant_file_header parsed_header;
    ...
    void parse_field(tag_t<field::alt> const & /**/, variant_alt_field & parsed_field) const
    {
    ...
    // make this back_insertable instead vector
    void parse_field(tag_t<field::alt> const & /**/, std::vector<variant_alt_field> & parsed_field) const
    {
        std::string_view raw_field = get<field::alt>(raw_record);
    ...
    void parse_field(tag_t<field::header_ptr> const & /**/, variant_file_header * & parsed_field) const
    {
        parsed_field = &parsed_header;
    }

    void parse_field(tag_t<field::header_ptr> const & /**/, std::string * & parsed_field) const
    {
        parsed_field = &raw_header;
    }

    void parse_field(tag_t<field::header_ptr> const & /**/, std::span<std::byte> * & parsed_field) const
    {
        parsed_field = &raw_header_as_bytes;
    }

    /* TODO move to mixin: */

    static variant_file_allele parse_alt_impl(std::string_view && in)
    {
    ...
    static variant_file_header parse_header(std::string_view raw_header)
    {
        variant_file_header ret;
